...
def mtt_checklist(price_df, rs):
    ...
    # 조건 (한글 설명)
    checks = [
        ("현재 주가가 150일선과 200일선 위에 있다.", close > ma150 and close > ma200),
        ("150일 이동평균선이 200일 이동평균선 위에 있다.", ma150 > ma200),
        ("200일 이동평균선이 최근 1개월 이상 상승 추세다.", ma200 > prev200),
        ("50일 이동평균선이 150일선과 200일선 위에 있다.", ma50 > ma150 and ma50 > ma200),
        ("현재 주가가 50일 이동평균선 위에 있다.", close > ma50),
        ("현재 주가가 52주(1년) 최저가 대비 30% 이상 높다.", (close - min_52w) / min_52w >= 0.3 if min_52w > 0 else False),
        ("현재 주가가 52주(1년) 최고가 대비 25% 이내에 있다.", (max_52w - close) / max_52w <= 0.25 if max_52w > 0 else False),
        ("상대강도(RS) 지수가 70 이상이다.", rs >= 70),
    ]
