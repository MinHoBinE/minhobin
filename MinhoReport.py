...
def get_latest_date(code, name):
    ...
    for i in range(30):
        ...
        print(f"[{check_date}-krx-rs.markdown] 확인 중, [{check_date}/{code}-{name}.csv] 확인 중")
...
def load_rs_from_markdown(date_str, code):
    ...
    if resp.status_code != 200:
        raise FileNotFoundError(f"RS 마크다운 파일을 찾을 수 없습니다: {url}")
...
def load_stock_price_csv(base_url, date, code, name):
    ...
    resp = requests.get(url)
    if resp.status_code != 200:
        raise FileNotFoundError(f"가격 데이터 파일을 찾을 수 없습니다: {url}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if rs_row.empty:
            print("RS 데이터에서 해당 종목을 찾을 수 없습니다.")
            exit(1)
